...
!       ut=400
      ...
      write(ut40,*) "phase 1 : Find the choke points", ns*(np-1)+1
      do sp=1,lnbr
         do j=1,nnbr
            m = (sp-1)*(nnbr-1)+j
...
!                    dg_pis(nnbr,lnbr,m,wx,xx,xx(mp))
        ...
        subroutine analytical_basis_reduced(workdir, s, i,            &
                           auxc1, auxc2, auxc3, auxc4, wfc, wxx)
        use util
        include 'open/files_index'
        integer :: aux0, krange
...
!       real*8 :: pis
        ...
        if (n.eq.((s-1)*(np-1)+np)) then
           ...
           if (r.lt.x(n)) then
              g_pis = dsqrt(ww(np)) * pis(np,ns,s,np,w,x,r)
           else
              g_pis = dsqrt(ww(1))  * pis(np,ns,s+1,1,w,x,r)
           end if

           g_pis = g_pis / dsqrt(ww(1)+ww(np))
        else
