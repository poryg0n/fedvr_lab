...
!     wfc0 = eigenvec(:,1)/wx/dsqrt(scaling)
      ...
      allocate(dx(nmax),dwfc(nmax),dwfc2(nmax), dwfc3(nmax))
      allocate(dwwfc(nmax))
      ...
      do i=1,nmax
         write(446,*) i, xx(i), basis(i,i), wfc0(i)
      enddo



      allocate(auxc1(nmax),auxc2(nmax))
      allocate(auxc3(nbpts),auxc4(nbpts),xt(nbpts))
      call write_analytical_basis(workdir)
      call compute_analytic_deriv(workdir, wx, xx, dwwfc,           &
                                                   wwfc, wfc0)
...
!       allocate(auxr1(nmax))
!       open(ut83, file='data/basis_vect.dat', status='unknown')
!      
!       do 
!          read(ut83,*,iostat=ios) auxr1
!          if (ios /= 0) exit
!          write(*,*) i, auxr1
!       enddo
...
!       do k=1,nbpts
!          xt(k) = xx1 + (k-1) * step
!          write(ut31,*) k, xt(k), real(auxc3(k)), imag(auxc3(k)),      &
!                                  real(auxc4(k)), imag(auxc4(k))
!       enddo 
!
!      auxc1 = auxc1 / wx / dsqrt(scaling)
!      do k=1,nmax
!         write(ut32,*) k, xx(k), real(auxc1(k)), imag(auxc1(k)),      &
!                                 real(auxc2(k)), imag(auxc2(k)),      &
!                           wfc0(k), -eigenvec(k,1)/wx(k)/dsqrt(scaling)
!      enddo 
...
!         call composite_trapeze_c(krange, kk, auxck2, auxcc1)
...
!         write(ut23,*) xx(i), dwfc(i), dwfc2(i),                       &
!             real(dwfc(i)),imag(dwfc(i)),real(dwfc2(i)),imag(dwfc2(i))
!      enddo
!
!
!
...
!
!
!! ***    purely homogeneous part
!!        auxc1 = c0           
!!        auxc2 = c0           
!         phi = phi - 0.5d0 * step * auxc1
!
!         call split_op(i,step,nmax,vextv,eigenvec,eigval,               &
!                        phi0,phi)
!
...
!
!!     do j=1,nmax
!!        write(ut31,*) time_(ij), j, xx(j), dsqrt(2.d0*eigval(j)),    &
...
!
!
!! ***  We try to compute the analytical derivative here
!!       do s=1,lnbr
!!          do i=1,nnbr
!! !           call analytical_basis(nnbr,lnbr,xmax,s,i)
!!             call analytical_basis(workdir,s,i)
!!          enddo
!!       enddo
!
!
...
!!!      auxck1 = c0
!!!      do k=1,krange
!!!          auxc = wfc * exp(-ci*kk(k)*xx) * wx * wx * scaling
!!!          do i=1,nmax
!!!             auxck1(k) = auxck1(k) + auxc(i)
!!!          enddo
!!!      enddo
...
!!
!!      do i=1,nmax
!!         write(ut24,*) xx(i), dwfc(i), dwfc2(i),                       &
!!             real(dwfc(i)),imag(dwfc(i)),real(dwfc2(i)),imag(dwfc2(i))
!!      enddo
!!
!!
